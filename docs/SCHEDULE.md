...
| 우선 | 작업                                                         | 영향                                 | 비용      |
| ---- | ------------------------------------------------------------ | ------------------------------------ | --------- |
| ★★★  | 야간 사이클 재실행 — 새 io_pin recipe + 디바이스 템플릿 검증 | F 카테고리 0→8 회복 확인             | 3~4h 야간 |
| ★★★  | morning_review 실행 — 누적 results.jsonl에서 ML signal 추출  | C/B 노이즈 비교, 진짜 의도 변경 식별 | 30분      |

### 핸들러 구현 (+6 시드 회복)

| 우선 | 작업                                                         | 영향                      |
| ---- | ------------------------------------------------------------ | ------------------------- |
| ★★   | `network_property` 핸들러 — NETWORK 노드 클릭 → BitRate 변경 | A: net_bitrate_change 0→3 |
| ★★   | `dialog_probe` → 실 핸들러 승격 — Pre-defined Index 자동화   | D: od_add_predefined 0→3  |

### 디버그 (기존 실패 시드 회복)

| 우선 | 작업                                                                                          | 영향         |
| ---- | --------------------------------------------------------------------------------------------- | ------------ |
| ★★   | A 카테고리 `toolbar_action_with_dialog` 디버그 (net_add_device 0/3, net_add_slave_device 0/3) | A: 6→12 회복 |
| ★★   | D 카테고리 OD dialog 처리 강화 (od_remove_first, od_store/restore/import/export)              | D: 9→24 회복 |
| ★    | E 카테고리 PDO 일부 실패 (pdo_remove_tx/rx)                                                   | E: 9→15 회복 |

### 근본 개선 (ML signal 정제)

| 우선 | 작업                                                                        | 영향                             |
| ---- | --------------------------------------------------------------------------- | -------------------------------- |
| ★★★  | Baseline 정합화 — MultiTool 로드 후 즉시 save한 깨끗한 .mtproject 백업 생성 | Save 부수효과 28→0 (ML 가능)     |
| ★★   | C 카테고리 재검증 — table_column fix 후 실제 Min/Max 값 변경 확인           | "OK 24건"이 진짜 의도된 변경인지 |

### 미정 (별도 probe 필요)

| 우선 | 작업                                                           | 영향               |
| ---- | -------------------------------------------------------------- | ------------------ |
| ★    | Variable Name 변경 경로 탐색 (핀 우클릭/더블클릭 → 다이얼로그) | F: +8 시드 추가    |
| ★    | 다른 디바이스 시드(3720, 5050, 6807) 일반화                    | 디바이스별 ×8 시드 |
