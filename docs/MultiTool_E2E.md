...
야간 자율 탐색(Qwen3.5) + 주간 자동 감독(Claude routine) 2-Tier 구조로 MultiTool GUI 전체 기능을 학습하고 `.exp` 파일을 자동 생성하는 E2E 에이전트.
...
│                              ┌─────────────┼────────────┐  │
│                              ▼             ▼            ▼  │
│                       Qwen3.5 (Mac)    pywinauto    xml_utils│
...
| 시간대      | 주체          | 역할                                                 | 비용 |
| ----------- | ------------- | ---------------------------------------------------- | ---- |
| 00:00~05:30 | Qwen3.5 (Mac) | 자율 탐색·실행·자기검증·KB 누적                      | 무료 |
...
- 유료 API 키 사용 금지 (야간) — 추론은 원격 Mac mini Ollama `qwen3.5:27b`(Apache 2.0)만 사용
- 야간 사이클 중 Claude Code 호출 금지 — Claude는 09:00 routine에서만 동작
- Qwen3.5의 직접 `verified=true` 승격 금지 — 야간은 `unverified`만, 승격은 Claude routine 권한
...
- Qwen3.5가 `kb/patterns/xml_rules.jsonl`·`exp_rules.jsonl` 직접 작성 금지 — 후보는 `_candidates.jsonl`에만, 승급은 Claude routine

## 검증조건

| 단계         | 검증 방법                                                                                 |
| ------------ | ----------------------------------------------------------------------------------------- |
| 백엔드 ready | `curl https://macmini.tailed5292.ts.net:11434/api/tags` → 200 + `qwen3.5:27b` 존재        |
| W1 PoC       | pywinauto 컨트롤 트리 JSON dump 성공 + Qwen3.5가 메뉴 분류 JSON 응답 반환                 |
...
| 항목         | 값                                                             |
| ------------ | -------------------------------------------------------------- |
| 추론 위치    | 원격 Mac mini 32GB (Tailscale)                                 |
| Base URL     | `https://macmini.tailed5292.ts.net:11434`                      |
| 현재 모델    | `qwen3.5:27b` (Qwen3.5 27.8B, Q4_K_M 17.4 GB, 2026-05-21 도입) |
| Capabilities | completion · **vision** · tools · thinking                     |
| 이전 모델    | `gemma4:26b` (Gemma 4 26B A4B MoE, 2026-04~05 사용)            |
| 라이선스     | Apache 2.0                                                     |
...
| Ollama Client    | 원격 Qwen3.5 호출·재시도·keep-alive       | `requests` + `tenacity`                |
...
**규칙**: Qwen3.5는 `verified=true` 직접 작성 금지. 모든 후보는 `kb/workflows.jsonl`에 `verified=false` + `source="nightly_<date>"`로 적재. 승급 권한은 주간 Claude routine.
...
야간: Qwen3.5가 신규 observations에 대해 일반화 규칙 후보를 LLM 추론으로 작성 → `kb/patterns/_candidates.jsonl`.
...
| 사전 | 완료 | `qwen3.5:27b` 헬스체크 200 OK 확인                                                                                                                            |
...
| W1   | 대기 | Qwen3.5에 트리 JSON 전송 → 최상위 메뉴 분류 PoC                                                                                                               |
